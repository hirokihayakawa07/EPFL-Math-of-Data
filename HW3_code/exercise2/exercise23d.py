...
## TO BE FILLED ##
# parameter setting
    Fx_true = fx(params['alpha_true'])+gx(params['alpha_true'])
    print('Fx_true', Fx_true)
    ...
    x = params['x0']
    lmbd = params['lambda']
    run_details = {'x_final': None, 'Fx_final': None, 'Fx_true': None, 'last_iter': None, 'iter': np.zeros(params['maxit']),'Fx': np.zeros(params['maxit']),'err': np.zeros(params['maxit'])}
# perform iteration
    for k in range(0, params['maxit']):
        x_next = proxg(x - alpha * gradf(x), alpha)
        err = abs(fx(x_next) + gx(x_next) - Fx_true)/ Fx_true
...
# perform iteration, swich model FISTA or FISTA-RESTART depending on input method name
    for k in range(0, params['maxit']):
        ...
        run_details['iter'][k] = k
        run_details['Fx'][k] = fx(x_next) + gx(x_next)
        run_details['err'][k] = err #err/err_0 for Ex 2.3 b
    run_details['x_final'] = x
    run_details['Fx_final'] = fx(x_next) + gx(x_next)
    run_details['Fx_true'] = Fx_true
    run_details['last_iter'] = k
...
def reconstruct_l1(image, indices, optimizer, params):
    ...
    params['alpha_true'] = r.W(params['x_true'])
...
if __name__ == "__main__":
    ...
    params['x_true'] = image # alpha_true = Wx is calculated in def reconstruct_l1 and Fx_true is calculated in each algorithms
 
    # store results calculated with each algorithms    
    # results of each method include: x_final, Fx_final, Fx_true, last_iteration number, iteration array, Fx array, error array
    results = dict()
    params['method'] = 'ISTA'
    ...
    for key in ['ISTA', 'FISTA', 'FISTA-RESTART']:
        if key in results:
            ax1.plot(np.array(range(0, results[key]['last_iter']+1)), (abs(results[key]['Fx'][:results[key]['last_iter']+1] - results[key]['Fx_true']) / results[key]['Fx_true']),
                                                                color=colors[key], lw=2, label=key)
    ax1.legend()
    ax1.set_xlabel('#iterations')
    ax1.set_ylabel(r'$ |f(\mathbf{x}^k) - f^\natural|  /  f^\natural$')
    ax1.set_ylim(1e-15, 1e2)
    ax1.set_yscale('log')
    ax1.grid()
    fig.tight_layout() 
    plt.suptitle('Exercise 2.3d')
    plt.show()