...
def l1_prox(y, weight):
    ############## YOUR CODES HERE ##############
    return multiply(maximum(absolute(y) - weight, 0), sign(y))
...
#     num_class = X.shape[1]
#     Z_d = sum(exp(A @ X), axis=1)
#     Z = [1/Z_d[i] for i in range(0, num_samples)]
#     Y = np.zeros((num_samples,num_class))
#     for i in range(0, num_samples):
#         Y[i,b[i]-1] = 1
    Z = diag(1/sum(exp(A @ X), axis=1))
    return transpose(A) @ (Z @ exp(A @ X) - b)
#     raise NotImplemented('Implement the method!')
...
def stocgradfx(X, minibatch_size, A, b):
    ...
    num_class = X.shape[1]
    ms = minibatch_size
    sgrad = np.zeros((X.shape[0],X.shape[1]))
#     Y = np.zeros((n,num_c))
#     for i in range(0, n):
#         Y[i,b[i]] = 1
    ...
    for j in range(0, ms):
        i = randint(0,ms-1)
        ai_T = A[i,:].reshape(1,d)
        ai = ai_T.reshape(d,1)
        bi = b[i,:].reshape(1,num_class)
        C = sum(exp(A@X), axis=1)
        sgrad = sgrad + n/ms * ai @ (exp(ai_T@X)/C[i] - bi)
